 ...
 *
 * Name(s):  Raymond Zou, Ritesh Kanchi
 * NetID(s): ryz, rkanchi
 *
 * NOTES:
...
void* mm_malloc(size_t size) {
  ...
  size_t block_size;
  size_t block_use_preceding_tag;

  block_info* new_free_block;
  block_info* follow_block;
  ...
  // searches free list for a free block that fits request
  ptr_free_block = search_free_list(req_size);
  
  //if free free not found, request more space and search again
  if(ptr_free_block == NULL){
	  request_more_space(req_size);
	  // search free list again for free block that fits request size
	  ptr_free_block = search_free_list(req_size);
  }

  //set tags (everything except size) for the allocated block
  block_use_preceding_tag = (ptr_free_block->size_and_tags & TAG_PRECEDING_USED)|TAG_USED;

  //remove free block from list
  remove_free_block(ptr_free_block);

  //set block size of the free block we found based on the size tag of block 
  block_size = SIZE(ptr_free_block->size_and_tags);

  //If difference between free block size and request size is atleast min block
  //size, then we split blocks and apply the relevant tags
  if (block_size - req_size >= MIN_BLOCK_SIZE){
	  //set new size and tags of newly allocated block
	  ptr_free_block->size_and_tags = req_size | block_use_preceding_tag;

	  //store address of new free block after split
	  new_free_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);

	  //set size and tags of new free block (following)
	  new_free_block->size_and_tags = (block_size-req_size) | TAG_PRECEDING_USED;

	  //set footer boundary tag of the new free block
	  *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE)) = new_free_block->size_and_tags;

	  //insert new free block into free block list
	  insert_free_block(new_free_block);
  } else { //Otherwise, no split and we just allocate the entire free block
	  //find pointer to following block
	  follow_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size);

	  //sets preceding tag of following block
	  follow_block -> size_and_tags |= TAG_PRECEDING_USED;

	  //sets tags of allocated block
	  ptr_free_block -> size_and_tags = block_size | block_use_preceding_tag;
  }
...
void mm_free(void* ptr) {
  ...
  //set pointer to header of block we want to free
  block_to_free = (block_info*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  //set size in header of block to free
  payload_size = SIZE(block_to_free->size_and_tags);
  //set pointer to following (adjacent block) block
  following_block = (block_info*) UNSCALED_POINTER_ADD(block_to_free, payload_size); 

  //set the preceding used tag of the following (adjacent) block using complement of bit mask
  //clear the preceding used tag bits 
  following_block->size_and_tags &= (~TAG_PRECEDING_USED);
  
  //set used tag block to free using complement of bit mask
  //clear the used tag bits
  block_to_free -> size_and_tags &= (~TAG_USED);

  //set footer of block to free
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free->size_and_tags;

  //insert block to free into free list
  insert_free_block(block_to_free);
  //coalesce new free block with any adjacent free blocks
